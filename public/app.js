...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // API isteği başlatılıyor (Timeout kontrolü ile)
        ...
        if (json.success && json.data) {
            console.log("Data received, rendering results...");
            resultsContainer.classList.remove('hidden');
            document.getElementById('dashboard-grid').classList.remove('hidden');

            renderResults(json.data);
        } else {
            throw new Error(json.error || "Sunucu veri döndürmedi.");
        }

    } catch (err) {
        console.error("Analysis Error:", err);
        errContainer.textContent = "Hata Oluştu: " + (err.name === 'AbortError' ? 'Zaman aşımı (60sn)' : err.message);
...
function renderResults(data) {
    ...
    try {
        ...
        if (tabIpIntel) {
            ...
            if (data.ip_intel && data.ip_intel.length > 0) {
                ...
                data.ip_intel.forEach(intel => {
                    intelHtml += `
                        <div class="record-row" style="display:flex; justify-content:space-between; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
        ...
        if (container && window.vis) {
            ...
            if (data.open_ports && (firstIp || (data.ip_intel && data.ip_intel.length > 0))) {
